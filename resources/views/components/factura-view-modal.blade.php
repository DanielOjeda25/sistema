...
<div x-show="verFactura" x-cloak class="fixed inset-0 overflow-y-auto" style="z-index: 9999" role="dialog" aria-modal="true" @keydown.escape.window="verFactura = null">
    <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm" @click="verFactura = null"></div>

    <div class="min-h-full flex items-center justify-center p-4">
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden" x-show="verFactura"
                ...
                <div>
                    ...
                    <div class="relative bg-gradient-to-br from-[#202225] to-[#2d3134] px-6 py-5 text-white">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="min-w-0">
                                <h3 class="text-lg font-bold leading-tight" x-text="verFactura.numero"></h3>
                                <p class="text-xs text-slate-300 mt-0.5 truncate" x-text="verFactura.proyecto"></p>
                            </div>
                            <div class="ml-auto text-right">
                                <p class="text-xl font-bold text-[#00e5a0]" x-text="verFactura.monto"></p>
                                <span class="mt-1 inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-semibold"
                                    :class="verFactura.estado === 'pagada' ? 'bg-emerald-400/20 text-emerald-300' : (verFactura.estado === 'vencida' ? 'bg-red-400/20 text-red-300' : 'bg-amber-400/20 text-amber-200')"
                    ...
                    <div class="px-6 py-2 divide-y divide-gray-100">
                        <div class="flex items-center gap-3 py-3.5">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-800" x-text="verFactura.emision"></p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3 py-3.5">
                            <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-[#008c63]">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-800" x-text="verFactura.vencimiento"></p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3 py-3.5">
                            <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-[#008c63]">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-sm font-medium text-gray-800 truncate" x-text="verFactura.emitida_por"></p>
                            </div>
                        </div>
                        <div class="flex items-start gap-3 py-3.5" x-show="verFactura.detalle">
                            <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-[#008c63]">
                                <x-heroicon-o-document-text class="h-5 w-5" />
                            </span>
                            <div class="min-w-0">
                                <p class="text-[11px] font-semibold uppercase tracking-wide text-gray-400">Detalle</p>
                                <p class="text-sm font-medium text-gray-800" x-text="verFactura.detalle"></p>
                            </div>
                        </div>
                    </div>

                    <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
                        <a :href="verFactura.pdf" class="inline-flex items-center gap-1.5 px-4 py-2 bg-[#00b87d] rounded-lg text-xs font-semibold text-white uppercase tracking-widest hover:bg-[#008c63]">
                            <x-heroicon-o-arrow-down-tray class="w-4 h-4" /> Descargar PDF
                        </a>
                        <button type="button" @click="verFactura = null"
